...
public class TTDVideoFiltersVC: UIViewController, IsMediaFilterVC {
    
    @IBOutlet weak var trimBottomItem: TTDTextMenuItem!
    @IBOutlet weak var coverBottomItem: TTDTextMenuItem!
    
    @IBOutlet weak var videoView: TTDVideoView!
    @IBOutlet weak var trimmerView: TrimmerView!
    
    @IBOutlet weak var coverImageView: UIImageView!
    @IBOutlet weak var coverThumbSelectorView: ThumbSelectorView!

    public var inputVideo: TTDMediaVideo!
    ...
    public class func initWith(video: TTDMediaVideo,
                               isFromSelectionVC: Bool) -> TTDVideoFiltersVC {
        let vc = TTDVideoFiltersVC(nibName: "TTDVideoFiltersVC", bundle: Bundle(for: TTDVideoFiltersVC.self))
        vc.inputVideo = video
    ...
    @objc public func save() {
        ...
        navigationItem.rightBarButtonItem = TTDLoaders.defaultLoader
                ...
                case .completed:
                    DispatchQueue.main.async {
                        if let coverImage = self?.coverImageView.image {
                            let resultVideo = TTDMediaVideo(thumbnail: coverImage, videoURL: destinationURL, asset: self?.inputVideo.asset)
                            didSave(TTDMediaItem.video(v: resultVideo))
                            self?.setupRightBarButtonItem()
                        } else {
                            print("TTDVideoFiltersVC -> Don't have coverImage.")
                        }
                    }
                case .failed:
                    print("TTDVideoFiltersVC -> Export of the video failed. Reason: \(String(describing: session.error))")
                default:
                    print("TTDVideoFiltersVC -> Export session completed with \(session.status) status. Not handling.")
                }
            }
        } catch let error {
            print("💩 \(error)")
    ...
    @objc public func selectCover() {
        title = TTDConfig.wordings.cover
...
extension TTDVideoFiltersVC: TrimmerViewDelegate {
...
extension TTDVideoFiltersVC: ThumbSelectorViewDelegate {

...
extension TTDLibraryVC {
    var isLimitExceeded: Bool { return selection.count >= TTDConfig.library.maxNumberOfItems }
    
    func setupCollectionView() {
        v.collectionView.backgroundColor = TTDConfig.colors.libraryScreenBackgroundColor
        v.collectionView.dataSource = self
        v.collectionView.delegate = self
        v.collectionView.register(TTDLibraryViewCell.self, forCellWithReuseIdentifier: "TTDLibraryViewCell")
    ...
    @objc func handleLongPress(longPressGR: UILongPressGestureRecognizer) {
        if multipleSelectionEnabled || isProcessing || TTDConfig.library.maxNumberOfItems <= 1 {
            return
        }
...
extension TTDLibraryVC: UICollectionViewDataSource {
    public func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        return mediaManager.fetchResult.count
    }
}

extension TTDLibraryVC: UICollectionViewDelegate {
    
    public func collectionView(_ collectionView: UICollectionView,
                               cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let asset = mediaManager.fetchResult[indexPath.item]
        guard let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "TTDLibraryViewCell",
                                                            for: indexPath) as? TTDLibraryViewCell else {
                                                                fatalError("unexpected cell in collection view")
        }
        cell.representedAssetIdentifier = asset.localIdentifier
        cell.multipleSelectionIndicator.selectionColor =
            TTDConfig.colors.multipleItemsSelectedCircleColor ?? TTDConfig.colors.tintColor
        ...
        cell.durationLabelContainer.isHidden = !isVideo
        cell.durationLabel.text = isVideo ? TTDHelper.formattedStrigFrom(asset.duration) : ""
        ...
        if let index = selection.firstIndex(where: { $0.assetIdentifier == asset.localIdentifier }) {
            ...
            if currentSelection.index < 0 {
                selection[index] = TTDLibrarySelection(index: indexPath.row,
    ...
    public func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        if multipleSelectionEnabled {
            ...
            //
            if let previousCell = collectionView.cellForItem(at: previouslySelectedIndexPath) as? TTDLibraryViewCell {
...
extension TTDLibraryVC: UICollectionViewDelegateFlowLayout {
    public func collectionView(_ collectionView: UICollectionView,
                               layout collectionViewLayout: UICollectionViewLayout,
                               sizeForItemAt indexPath: IndexPath) -> CGSize {
        let margins = TTDConfig.library.spacingBetweenItems * CGFloat(TTDConfig.library.numberOfItemsInRow - 1)
        let width = (collectionView.frame.width - margins) / CGFloat(TTDConfig.library.numberOfItemsInRow)
        return CGSize(width: width, height: width)
    }

    public func collectionView(_ collectionView: UICollectionView, layout collectionViewLayout: UICollectionViewLayout, minimumLineSpacingForSectionAt section: Int) -> CGFloat {
        return TTDConfig.library.spacingBetweenItems
    }

...
class TTDVideoCaptureHelper: NSObject {
    ...
    private let sessionQueue = DispatchQueue(label: "TTDVideoVCSerialQueue")
    ...
    public func zoom(began: Bool, scale: CGFloat) {
       ...
       do {
           ...
           maxAvailableVideoZoomFactor = min(maxAvailableVideoZoomFactor, TTDConfig.maxCameraZoomFactor)
    ...
    public func startRecording() {
        ...
        let outputURL = TTDVideoProcessor.makeVideoPathURL(temporaryFolder: true, fileName: "recordedVideoRAW")
    ...
    private func setupCaptureSession() {
        ...
        if let videoInput = videoInput {
            ...
            if (TTDConfig.video.fileType == .mp4) {
...
extension TTDVideoCaptureHelper: AVCaptureFileOutputRecordingDelegate {
    ...
    public func fileOutput(_ captureOutput: AVCaptureFileOutput,
                           ...
                           from connections: [AVCaptureConnection],
                           error: Error?) {
        TTDVideoProcessor.cropToSquare(filePath: outputFileURL) { [weak self] url in
